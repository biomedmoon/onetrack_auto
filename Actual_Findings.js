javascript:(function(){
  ...
  const GLOBAL_PHRASES = ["Repair Estimate under pre-approved limit."];
  ...
  function getPhrasesForDevice(dt) {
    ...
    let dp = m[dt] && m[dt].length > 0 ? m[dt] : (
      dt.includes('Solis VIP PharmGuard Pump') ? [
  ...
  function showMainModal() {
    ...
    db.onclick = () => { ov.remove(); showDeclineActionModal(); };
    cont.appendChild(db);
    ...
    ab.onclick = () => { ov.remove(); showClientSelectModal('Repairs approved', false); };
    cont.appendChild(ab);
    ...
    eb.onclick = () => { ov.remove(); showEditModal(); };
    box.appendChild(eb);
  ...
  function showDeclineActionModal() {
    ...
    [
      {label: "Asked to be disposed of here at biomed facility", actionText: "Repairs declined and asked to be disposed of"},
      {label: "Asked to be returned unrepaired", actionText: "Repairs declined and asked to be returned"}
    ].forEach(item => {
      let btn = document.createElement('button');
  ...
  function showClientSelectModal(ap, sao) {
    ...
    if (sao) {
      ...
      configClientBtn.onclick = () => { ov.remove(); showEditModal(); };
    ...
    if (!sao) {
      ...
      ecBtn.onclick = () => { ov.remove(); showEditModal(); };
  ...
  function showEditModal() {
    ...
    function rcm() {
      ...
      Object.keys(map).sort((a, b) => a === 'Other' ? 1 : b === 'Other' ? -1 : a.localeCompare(b)).forEach(comp => {
        ...
        map[comp].forEach((cn, ci) => {
          ...
          cd.onclick = () => { map[comp].splice(ci, 1); rcm(); };
          r.appendChild(cd);
        ...
        acb.onclick = () => { map[comp].push("New Client"); rcm(); };
        cb.appendChild(acb);
    ...
    addCompBtn.onclick = () => {
      let ncName = prompt("Enter new company name:", "");
      if (ncName && ncName.trim()) {
        let cleanName = ncName.trim();
        if (!map[cleanName]) {
          map[cleanName] = [];
          rcm();
        } else {
    ...
    bc.style.cssText = "display:flex;justify-content:flex-end;gap:10px;";

    let cBtn = document.createElement('button');
    cBtn.innerText = "Cancel";
    cBtn.style.cssText = "padding:6px 12px;background:#e0e0e0;border:none;border-radius:4px;cursor:pointer;font-size:12px;";
    cBtn.onclick = () => showMainModal();
    bc.appendChild(cBtn);

    let sBtn = document.createElement('button');
    sBtn.innerText = "Save Configuration";
    sBtn.style.cssText = "padding:6px 12px;background:#780034;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:12px;font-weight:bold;";
    sBtn.onclick = () => {
      let cp = ph.map(l => l.trim()).filter(Boolean),
          sn = localStorage.getItem('device_preset_notes') ? JSON.parse(localStorage.getItem('device_preset_notes')) : {};
      sn[dt] = cp;
      localStorage.setItem('device_preset_notes', JSON.stringify(sn));
      localStorage.setItem('my_preset_client_mapping', JSON.stringify(map));
      ov.remove();
      showMainModal();
    };
  ...
  function processTextSelection(str) {
    ...
    let mb = document.querySelector('#addActualFindingsModal .msd-modal-body, #addActualFindingsModal'),
        tgt = mb ? mb.querySelector('textarea, input[type="text"]:not([readonly])') : null,
        iam = localStorage.getItem('preset_notes_append_mode') === 'true';

    if (tgt) {
      if (iam && tgt.value.trim() !== '') {
        tgt.value = tgt.value.trim() + ' ' + str;
      } else {
        tgt.value = str;
      }
      tgt.focus();
      ...
      showToast(iam ? 'Appended into Pop-up!' : 'Pasted into Pop-up!');
    } else {
  ...
  function showToast(msg) {
    ...
    t.style.cssText = 'position:fixed;bottom:20px;right:20px;background:#780034;color:#fff;padding:12px 20px;border-radius:6px;z-index:9999999;font-family:sans-serif;font-size:13px;box-shadow:0 2px 8px rgba(0,0,0,0.3);font-weight:bold;';
    t.innerText = msg;
